...
class Calendario extends Database{
    ...
    public function criarEvento($id=0,$id_user=0, $title,$description, $start, $end, $sala){
        $this->db->query("INSERT INTO eventos(id_evento, id_solicitador, titulo_evento, descricao_evento, comeco_evento, fim_evento, sala_evento) values(?,?,?,?,?,?,?)");
    ...
    public function deletarEventoPendente($id){
        $this->db->query("DELETE FROM eventosPendentes WHERE id_evento=?");
        $this->db->bind(1,$id, \PDO::PARAM_INT);
        $this->db->executa();
    ...
    public function lerEventosPendentes(){
        $this->db->query("SELECT * FROM eventosPendentes");
        return $this->db->resultados();
    }
    public function solicitarEvento($id=0,$id_user=0, $title,$description, $start, $end, $sala){
        $this->db->query("INSERT INTO eventosPendentes(id_evento, id_solicitador, titulo_evento, descricao_evento, comeco_evento, fim_evento, sala_evento) values(?,?,?,?,?,?,?)");
    ...
    public function aprovar($id,$id_user, $title,$description, $start, $end, $sala){
        $this->db->query("INSERT INTO eventos( id_solicitador, titulo_evento, descricao_evento, comeco_evento, fim_evento, sala_evento) values(?,?,?,?,?,?)");

        $this->db->bind(1,$id_user, \PDO::PARAM_INT);
        $this->db->bind(2,$title, \PDO::PARAM_STR);
        $this->db->bind(3,$description, \PDO::PARAM_STR);
        $this->db->bind(4,$start, \PDO::PARAM_STR);
        $this->db->bind(5,$end, \PDO::PARAM_STR);
        $this->db->bind(6,$sala, \PDO::PARAM_STR);
        $this->db->executa();
